...
def test_render_internal_host_expanded(monkeypatch):
...
def test_external_override(monkeypatch):
    monkeypatch.setenv(
        "DATABASE_EXTERNAL_URL",
        "postgresql://u:p@dpg-abc-a.frankfurt-postgres.render.com/yombal",
    )
    url = "postgresql://u:p@dpg-abc-a/yombal"
    assert "frankfurt-postgres.render.com" in normalize_database_url(url)


def test_full_host_unchanged():
